...
public class PricePlanService {
    ...
    public Optional<Map<String, BigDecimal>> getusageCostOfElectricityReadingsForEachPricePlan(String smartMeterId) {
        ...
        return Optional.of(pricePlans.stream().collect(
                Collectors.toMap(PricePlan::getPlanName, t -> calculateUsageCost(electricityReadings.get(), t))));
    }
    
    public BigDecimal calculateUsageCost(List<ElectricityReading> electricityReadings, PricePlan pricePlan) {
    	
    	BigDecimal average = calculateAverageReading(electricityReadings);
    	BigDecimal timeElapsed = calculateTimeElapsed(electricityReadings);
    	BigDecimal engergyConsumed = average.multiply(timeElapsed);
    	
		return engergyConsumed;
